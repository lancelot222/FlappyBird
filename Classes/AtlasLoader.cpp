...
void AtlasLoader::loadAtlas(const std::string& filename)
{
    auto textureAtlas = Director::getInstance()->getTextureCache()->addImage("atlas.png");
    this->loadAtlas(filename, textureAtlas);
}

void AtlasLoader::loadAtlas(const std::string& filename, Texture2D *texture)
{
...
SpriteFrame* AtlasLoader::getSpriteFrameByName(const std::string& name)
{
